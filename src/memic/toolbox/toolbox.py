...
class Toolbox:
    ...
    def record(self):
        wav = self.ui.record_one(encoder.sampling_rate, self.ui.record_duration_input.value())
        if wav is None:
            return
        self.ui.play(wav, encoder.sampling_rate)

        speaker_name = self.ui.user_name_input.text() or "user01"
        name = self.ui.record_name_input.text() or speaker_name + "@rec01"

        # save the wav
        wav_path = Path(self.datasets_root, speaker_name, name + ".wav")
        wav_path.parent.mkdir(exist_ok=True, parents=True)
        write(wav_path, Synthesizer.sample_rate, wav)
        self.ui.check_filename()
    ...
    def init_encoder(self):
        ...
        self.ui.log("Done (%dms)." % int(1000 * (time.perf_counter() - start)), "append")
        self.ui.set_loading(0)
